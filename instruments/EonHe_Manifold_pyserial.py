...
class EonHeManifold():
    ...
    def __init__(self, port='COM3'):
        serial_port = serial.Serial(port, 9600, timeout=1000)
    ...
    #                                               timeout=timeout, **kwargs)
        ...
        # self.serial_port = SerialInstrument(name="helium_manifold_serial", address=port, baudrate=9600,
        #                                         enabled=True, recv_length=1024, timeout=1,
        #                                         query_sleep=0.10)
        # SerialInstrument.__init__(self, name="helium_manifold_serial", address=port, baudrate=9600,
        #                                         enabled=True, recv_length=1024, timeout=0.15,
        #                                         query_sleep=0.10)
    ...
    def open_valve(self, valve_number):
        self.serial_port.write("O%d\n"%valve_number)

    def close_valve(self, valve_number):
        self.serial_port.write("C%d\n"%valve_number)
    ...
    def get_transducer_voltage(self):
        self.serial_port.write("S3\n")
        # print(self.PressureReadOut.query_sleep)
        time.sleep(0.1)
        answer = self.serial_port.read()
        print("voltage:")
...
if __name__ == '__main__':
    ...
    heman.set_gas(state=True, override=True)